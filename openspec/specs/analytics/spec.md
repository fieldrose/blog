...
## Purpose
提供隐私优先、零第三方依赖的自建统计闭环：浏览器以无 Cookie 方式上报页面浏览与 Core Web Vitals，BFF 校验后经 service role 写入 Postgres，明细短期保留、聚合长期沉淀，并通过只读视图驱动公开的 `/zh|/en/stats` 仪表盘；同时覆盖限流、RLS 全量收敛与 Referer 脱敏等隐私要求。

## Requirements

### Requirement: 采集端点校验与响应

站点 MUST 提供 `POST /api/analytics/collect`（`prerender=false`），接受两类 JSON 负载：`pageview`（含 `clientId`、`path`、`locale`、`viewport`、`uaClass`，可选 `referrer`）与 `web-vital`（含 `clientId`、`path`、`metric`、`value`、`rating`、`id`）。`clientId` MUST 为 UUID（放请求体，因 `sendBeacon` 无法设置自定义头）。服务端 MUST 用 Zod 判别联合校验：`path` MUST 为站内绝对路径且长度受限，`metric` MUST 为 `lcp|cls|inp` 之一，`rating` MUST 为 `good|needs-improvement|poor` 之一。合法请求 MUST 返回 202；校验失败 MUST 返回 400 `VALIDATION_ERROR`；Supabase 未配置时 MUST 返回 503 `SUPABASE_NOT_CONFIGURED`。同一访客哈希每分钟超过 120 次写时 MUST 返回 429 `RATE_LIMITED`。
